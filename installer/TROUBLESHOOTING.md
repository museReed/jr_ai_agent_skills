...
| 13 | Codex：tab 名字閃一下，1 秒內又跳回資料夾名（如 `job`） | `codex doctor` 的 `title` 是不是 `enabled`（Codex 內建 terminal title 預設含 cwd，每回合覆寫 watcher 送的 OSC） | 重新執行最新版 `install.sh codex --editor=...`；installer 會保留既有 TOML 設定並加入 `[tui] terminal_title = []`。重啟 Codex 後用 `codex doctor` 確認 `title: disabled`。 |
